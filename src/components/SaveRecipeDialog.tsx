  ...
  async function save(mode: "update" | "new") {
    ...
    try {
      if (mode === "update" && s.savedRecipe) {
        const res = await updateRecipeFn({
          data: { id: s.savedRecipe.id, recipe },
        });
        if (res.ok) {
      ...
      setError({
        error: "No se pudo guardar la plantilla. Inténtalo de nuevo.",
      });
      setSaving(null);
    }
  }
    ...
    >
      ...
      >
        ...
        <TextInput
          ...
          onKeyDown={(e) =>
            e.key === "Enter" && !saving && save(savedRecipe ? "update" : "new")
          }
